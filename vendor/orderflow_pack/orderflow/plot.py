...
def render_layered_chart(book_df: pd.DataFrame, aggregated_trade_df: pd.DataFrame, ohlc_data: pd.DataFrame, oi_ohlc: pd.DataFrame, markers, cfg, market: str = 'Futures', symbol: str = 'BTC/USDT') -> tuple[io.BytesIO, int]:
    ...
    with plt.style.context('dark_background'):
        ...
        if latest_ohlc_visible is not None:
            ...
            ax_main_price.text(0.015, 0.975, f'{latest_plot_price:.2f}', transform=ax_main_price.transAxes, fontsize=max(rt.cp.TICK_LABEL_FONTSIZE * 2.2, 18), fontweight='bold', color=latest_price_color, va='top', ha='left', bbox=dict(boxstyle='round,pad=0.28', fc='black', ec=latest_price_color, lw=1.0, alpha=0.82), zorder=6)
