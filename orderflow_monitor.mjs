...
function trimFileToTailBytes(filePath, keepBytes) {
  if (!filePath || keepBytes <= 0) return;
  let size = 0;
  try {
  ...
  try {
    ...
    fs.writeFileSync(tmpPath, buf.subarray(from));
...
function safeWriteHealth(reason = '') {
  ...
  try {
    ...
    const payload = {
      ...
      lastDepthAppliedAt: state.lastDepthAppliedAt ? new Date(state.lastDepthAppliedAt).toISOString() : null,
...
function isMalformedDepthEvent(msg) {
  if (!msg || typeof msg !== 'object') return true;
  if (typeof msg.U !== 'number' || typeof msg.u !== 'number' || typeof msg.pu !== 'number') return true;
  if (msg.U <= 0 || msg.u <= 0 || msg.pu < 0) return true;
  if (msg.U > msg.u || msg.pu > msg.u) return true;
  if (!Array.isArray(msg.b) || !Array.isArray(msg.a)) return true;
  return false;
}
...
async function initSnapshotAndSync() {
  ...
  for (let pass = 0; pass < 5; pass++) {
    const pending = state.depthBuffer
      .filter((e) => !isMalformedDepthEvent(e) && e.u > state.lastUpdateId)
      .sort((a, b) => a.u - b.u);

    let start = pending.findIndex((e) => e.pu === state.lastUpdateId);
    if (start === -1) {
      start = pending.findIndex((e) => e.U <= state.lastUpdateId + 1 && e.u >= state.lastUpdateId + 1);
    }

    if (start === -1) break;

    const before = state.lastUpdateId;
    for (let i = start; i < pending.length; i++) {
      const e = pending[i];
      if (e.pu !== state.lastUpdateId) break;
      applyDepthEvent(e);
      progressed = true;
    }

    state.depthBuffer = state.depthBuffer.filter((e) => e.u > state.lastUpdateId);
    if (state.lastUpdateId === before) break;
  }

  if (!progressed && state.depthBuffer.length) {
    // keep buffer for the next sync cycle instead of dropping it on the floor
    state.depthBuffer = state.depthBuffer.filter((e) => e.u > state.lastUpdateId);
  }
...
function handleDepth(msg) {
  ...
  if (isMalformedDepthEvent(msg)) {
    sequenceGapCount += 1;
    const reason = `malformed-depth last=${state.lastUpdateId} pu=${msg?.pu} U=${msg?.U} u=${msg?.u} count=${sequenceGapCount}`;
    if (strictSeq || sequenceGapCount >= 3) {
      void resyncDepth(reason);
      return;
    }
    safeWriteHealth('malformed-depth');
    return;
  }
  ...
  const puMismatch = msg.pu !== state.lastUpdateId;
  const warnOnlyGap = msg.U > state.lastUpdateId + 1;
  if (puMismatch) {
    sequenceGapCount += 1;
    const reason = `sequence-gap last=${state.lastUpdateId} pu=${msg.pu} U=${msg.U} u=${msg.u} count=${sequenceGapCount}`;
    if (strictSeq || sequenceGapCount >= 3) {
      void resyncDepth(reason);
      return;
    }
    safeWriteHealth('gap-detected');
  } else {
    sequenceGapCount = 0;
    if (warnOnlyGap) {
      safeWriteHealth('u-gap-warning');
    }
  }

  applyDepthEvent(msg);
  state.lastDepthAppliedAt = Date.now();
}
...
function watchdog() {
  ...
  if (state.bookReady && state.lastDepthMsgAt && now - state.lastDepthMsgAt > staleDepthMs) {
    void resyncDepth(`stale-depth ${now - state.lastDepthAppliedAt}ms`);
  } else if (!state.bookReady && !state.syncing && lastResyncAt > 0 && now - lastResyncAt > staleDepthMs) {
    void resyncDepth(`snapshot-retry ${now - lastResyncAt}ms`);
  }
